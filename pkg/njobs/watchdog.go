...
func (w *Watchdog) step(ctx context.Context) error {
	wantSleep, err := w.evalSessionExpire(ctx, time.Now().Unix(), int64(w.Collection.SessionExpireBatch))
	if err != nil {
		return fmt.Errorf("failed to run session expire script: %w", err)
	}
	var sleep time.Duration
	if wantSleep > w.Collection.SessionExpireInterval {
		sleep = w.Collection.SessionExpireInterval
	} else {
		sleep = wantSleep
	}
